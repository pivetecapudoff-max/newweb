...
export interface SilkShaderBackgroundProps {
  className?: string;
  fixed?: boolean;
}
...
export function SilkShaderBackground({
  ...
  useEffect(() => {
    ...
    // Plain WebGL1 context with no libraries
    const gl =
    ...
    if (!gl) {
      console.warn("WebGL1 not supported for Silk shader background.");
      return;
    }
    ...
    // Fullscreen triangle positions in clip space: covers [-1, 1] x [-1, 1]
    ...
    // Static packed uniforms per spec:
    // u_shape = vec4(scale, intensity, paramA, warp) = vec4(1.26, 0.28, 0.50, 0.00)
    if (uShapeLoc) {
      gl.uniform4f(uShapeLoc, 1.26, 0.28, 0.50, 0.00);
    }
    // u_surface = vec4(detail, contrast, brightness, saturation) = vec4(2.40, 1.11, 0.00, 1.00)
    if (uSurfaceLoc) {
      gl.uniform4f(uSurfaceLoc, 2.40, 1.11, 0.00, 1.00);
    }
    // u_finish = vec4(hue, vignette, blur, grain) = vec4(0.00, 0.00, 0.000, 0.05)
    if (uFinishLoc) {
      gl.uniform4f(uFinishLoc, 0.00, 0.00, 0.000, 0.05);
    }
    // u_transform = vec4(seed, rotation, drift, OKLab toggle) = vec4(1581.0, 0.00, 0.00, 0.0)
    if (uTransformLoc) {
      gl.uniform4f(uTransformLoc, 1581.0, 0.00, 0.00, 0.0);
    }
    // u_space = vec4(offset.xy, pointer.xy) = vec4(0.00, 0.00, pointer x, pointer y)
    if (uSpaceLoc) {
      gl.uniform4f(uSpaceLoc, 0.00, 0.00, 0.0, 0.0);
    }
    // u_cursor = vec4(presence, 2.0, 0.65, 0.46) -> Cursor: off => presence = 0.0
    if (uCursorLoc) {
    ...
    // Animation & RAF loop with Tab Visibility Pause
  ...
  return (
    <canvas
      ...
      } inset-0 w-full h-full pointer-events-none ${className}`}
      style={{
